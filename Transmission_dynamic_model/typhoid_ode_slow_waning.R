...
# 
# Function containing the Ordinary Differential Equations used in the ODE solver in the function "parallelization_slow"
#
# Created: Catherine Wenger Jan 2025
###############################################################################

typhoid_ode_slow <- function(t,y,parameters){
  
  al = parameters$al                          # number of age groups

  # Set up states
  dydt = vector(len = 23*al)
  ...
  omegav0 = parameters$omegav0                # Duration of vaccine protection for individuals vaccinated at 0-<2 years
  omegav1 = parameters$omegav1                # Duration of vaccine protection for individuals vaccinated at 2-<5 years
  omegav2 = parameters$omegav2                # Duration of vaccine protection for individuals vaccinated at 5+ years
  ...
  ## [3] Aging processes + routine and booster vaccinations (which are administered as part of aging)
  ...
  for(i in 1:al){
        ...
        V_UNDER_2_V1 = y[i+12*al] # includes routine vaccinees and campaign vaccinees under 2 at time of campaign
        V_UNDER_2_V2 = y[i+13*al] # includes routine vaccinees and campaign vaccinees under 2 at time of campaign (prior infection)
        V_2TO5_V1 = y[i+14*al]    # includes routine vaccinees and campaign vaccinees 2-5 at time of campaign
        V_2TO5_V2 = y[i+15*al]    # includes routine vaccinees and campaign vaccinees 2-5 at time of campaign (prior infection)
        V_OVER_5_V1 = y[i+16*al]  # includes routine/booster recipients and campaign recipients over 5 at time of campaign
        V_OVER_5_V2 = y[i+17*al]  # includes routine/booster recipients and campaign recipients over 5 at time of campaign (prior infection)
        ...
        dSV1 = omegav0*V_UNDER_2_V1 + omegav1*V_2TO5_V1 + omegav2*V_OVER_5_V1 + epsilon*SV2 - (lambdap[i]+lambdaw)*SV1 - (u[i]+mu[i])*SV1
        dIV1 = (lambdap[i]+lambdaw)*SV1 + (lambdap[i]+lambdaw)*(1-VE0)*V_UNDER_2_V1 + (lambdap[i]+lambdaw)*(1-VE1)*V_2TO5_V1 + (lambdap[i]+lambdaw)*(1-VE2)*V_OVER_5_V1 - delta*(1-alpha-theta[i])*IV1 - delta*theta[i]*IV1 - delta*alpha*IV1 - (u[i]+mu[i])*IV1
        dRV = delta*(1-alpha-theta[i])*IV1 + delta*(1-theta2[i])*IV2 - omega*RV - (u[i]+mu[i])*RV
        dSV2 = omegav0*V_UNDER_2_V2 + omegav1*V_2TO5_V2 + omegav2*V_OVER_5_V2 + omega*RV - epsilon*SV2 - (lambdap[i]+lambdaw)*SV2 - (u[i]+mu[i])*SV2
        dIV2 = (lambdap[i]+lambdaw)*SV2 + (lambdap[i]+lambdaw)*(1-VE0)*V_UNDER_2_V2 + (lambdap[i]+lambdaw)*(1-VE1)*V_2TO5_V2 + (lambdap[i]+lambdaw)*(1-VE2)*V_OVER_5_V2 - delta*IV2 - (u[i]+mu[i])*IV2
        dCV = delta*(theta[i]*IV1 + theta2[i]*IV2) - (u[i]+mu[i])*CV

        # VACCINATION CLASSES
        # If vaccinated at a time when they were <2 years of age
        dV_UNDER_2_V1 = -omegav0*V_UNDER_2_V1 - (u[i]+mu[i])*V_UNDER_2_V1 - (lambdap[i]+lambdaw)*(1-VE0)*V_UNDER_2_V1 
        dV_UNDER_2_V2 = -omegav0*V_UNDER_2_V2 - (u[i]+mu[i])*V_UNDER_2_V2 - (lambdap[i]+lambdaw)*(1-VE0)*V_UNDER_2_V2
        
        # If vaccinated at a time when they were 2-<5 years of age
        dV_2TO5_V1 = -omegav1*V_2TO5_V1 - (u[i]+mu[i])*V_2TO5_V1 - (lambdap[i]+lambdaw)*(1-VE1)*V_2TO5_V1 
        dV_2TO5_V2 = -omegav1*V_2TO5_V2 - (u[i]+mu[i])*V_2TO5_V2 - (lambdap[i]+lambdaw)*(1-VE1)*V_2TO5_V2

        # If vaccinated at a time when they were 5 or older
        dV_OVER_5_V1 = -omegav2*V_OVER_5_V1 - (u[i]+mu[i])*V_OVER_5_V1 - (lambdap[i]+lambdaw)*(1-VE2)*V_OVER_5_V1
        dV_OVER_5_V2 = -omegav2*V_OVER_5_V2 - (u[i]+mu[i])*V_OVER_5_V2 - (lambdap[i]+lambdaw)*(1-VE2)*V_OVER_5_V2
        ...
        dCuml_IV1 = (lambdap[i] + lambdaw) * SV1 + (lambdap[i]+lambdaw)*(1-VE0)*V_UNDER_2_V1 + (lambdap[i]+lambdaw)*(1-VE1)*V_2TO5_V1 + (lambdap[i]+lambdaw)*(1-VE2)*V_OVER_5_V1


        # >>> BLOCK [2] VACCINATION: CAMPAIGNS
        # If out of the burn-in period and into the vaccination time
        # This when vaccination begins
        ...
        if(round(t) > END_OF_BURN_IN){
            ...
            if(i %in% c(1,2,3)){
              ...
              dV_UNDER_2_V1 = dV_UNDER_2_V1 + S1*(vaxnc[round(t),i])      # no prior infection
              dV_UNDER_2_V2 = dV_UNDER_2_V2 + (S2+R)*(vaxnc[round(t),i])  # prior infection 
              ...
              # people who were vaccinated in the catch-up campaign moving into vaccinated class based on prior infection
              dV_2TO5_V1 = dV_2TO5_V1 + S1*(vaxnc[round(t),i])      # no prior infection 
              dV_2TO5_V2 = dV_2TO5_V2 + (S2+R)*(vaxnc[round(t),i])  # prior infection 
              ...
              # people who were vaccinated in the catch-up campaign moving into vaccinated class based on prior infection
              dV_OVER_5_V1 = dV_OVER_5_V1 + S1*(vaxnc[round(t),i])      # no prior infection
              dV_OVER_5_V2 = dV_OVER_5_V2 + (S2+R)*(vaxnc[round(t),i])  # prior infection
            }
        ...
        # >>> BLOCK [3] AGING + BOOSTER AND ROUTINE VACCINES
        ...
        # THEREFORE, we only need to start beyond the first age group
            if (i>1){
              ...
              V_UNDER_2_V1_prior = y[i+12*al-1]
              V_UNDER_2_V2_prior = y[i+13*al-1]
              V_2TO5_V1_prior = y[i+14*al-1]
              V_2TO5_V2_prior = y[i+15*al-1]
              V_OVER_5_V1_prior = y[i+16*al-1]
              V_OVER_5_V2_prior = y[i+17*al-1]

              # >>> | ROUTINE DOSES | <<<
              # >>> UNVACCINATED FOLKS <<<
              ## This is for the folks who don't get vaccinated at all =  1 - Vaccination Coverage (routine)
              ...
              dV_UNDER_2_V1 = dV_UNDER_2_V1 + u[i-1]*V_UNDER_2_V1_prior
              dV_UNDER_2_V2 = dV_UNDER_2_V2 + u[i-1]*V_UNDER_2_V2_prior
              dV_2TO5_V1 = dV_2TO5_V1 + u[i-1]*V_2TO5_V1_prior
              dV_2TO5_V2 = dV_2TO5_V2 + u[i-1]*V_2TO5_V2_prior
              dV_OVER_5_V1 = dV_OVER_5_V1 + u[i-1]*V_OVER_5_V1_prior
              dV_OVER_5_V2 = dV_OVER_5_V2 + u[i-1]*V_OVER_5_V2_prior
              ...
              if(i %in% c(2,3)){
                ...
                dV_UNDER_2_V1 = dV_UNDER_2_V1 + u[i-1]*S1_prior*(vaxr[round(t),(i-1)])
                dV_UNDER_2_V2 = dV_UNDER_2_V2 + u[i-1]*(S2_prior+R_prior)*(vaxr[round(t),(i-1)])
              } else if(i %in% c(4)){
                # Individuals from S/R compartments aging into vaccine eligible age group and getting vaccinated (2-5 yr group)
                dV_2TO5_V1 = dV_2TO5_V1 + u[i-1]*S1_prior*(vaxr[round(t),(i-1)])
                dV_2TO5_V2 = dV_2TO5_V2 + u[i-1]*(S2_prior+R_prior)*(vaxr[round(t),(i-1)])
              } else if(i %in% c(5:al)){
                # Individuals from S/R compartments aging into vaccine eligible age group and getting vaccinated (>5 yr group)
                dV_OVER_5_V1 = dV_OVER_5_V1 + u[i-1]*S1_prior*(vaxr[round(t),(i-1)])
                dV_OVER_5_V2 = dV_OVER_5_V2 + u[i-1]*(S2_prior+R_prior)*(vaxr[round(t),(i-1)])
              }
              ...
              if(i %in% c(2,3)){
                ...
                dV_UNDER_2_V1 = dV_UNDER_2_V1 + u[i-1]*SV1_prior*(vaxb[round(t),(i-1)])
                dV_UNDER_2_V2 = dV_UNDER_2_V2 + u[i-1]*(SV2_prior + RV_prior)*(vaxb[round(t),(i-1)])
                } else if(i %in% c(4)){
                  # Individuals from S/R compartments aging into vaccine eligible age group and getting vaccinated (2-5 yr group)
                dV_2TO5_V1 = dV_2TO5_V1 + u[i-1]*SV1_prior*(vaxb[round(t),(i-1)])
                dV_2TO5_V2 = dV_2TO5_V2 + u[i-1]*(SV2_prior + RV_prior)*(vaxb[round(t),(i-1)])
                } else if(i %in% c(5:al)){
                  # Individuals from S/R compartments aging into vaccine eligible age group and getting vaccinated (>5 yr group)
                dV_OVER_5_V1 = dV_OVER_5_V1 + u[i-1]*SV1_prior*(vaxb[round(t),(i-1)])
                dV_OVER_5_V2 = dV_OVER_5_V2 + u[i-1]*(SV2_prior + RV_prior)*(vaxb[round(t),(i-1)])
                }
              # Cumulative vaccinations booster
              dCuml_Vax_Booster_Doses = dCuml_Vax_Booster_Doses + u[i-1]*(SV1_prior + IV1_prior + RV_prior + SV2_prior + IV2_prior + CV_prior + V_UNDER_2_V1_prior + V_UNDER_2_V2_prior + V_2TO5_V1_prior + V_2TO5_V2_prior + V_OVER_5_V1_prior + V_OVER_5_V2_prior)*(vaxb[round(t),(i-1)])
        ...
        dydt[i+10*al] = dIV2
        dydt[i+11*al] = dCV
        dydt[i+12*al] = dV_UNDER_2_V1
        dydt[i+13*al] = dV_UNDER_2_V2
        dydt[i+14*al] = dV_2TO5_V1
        dydt[i+15*al] = dV_2TO5_V2
        dydt[i+16*al] = dV_OVER_5_V1
        dydt[i+17*al] = dV_OVER_5_V2
        dydt[i+18*al] = dCuml_Vax_Camp_Doses
        dydt[i+19*al] = dCuml_Vax_Booster_Doses
        dydt[i+20*al] = dCuml_Vax_Routine_Doses
        dydt[i+21*al] = dCuml_I1
        dydt[i+22*al] = dCuml_IV1
