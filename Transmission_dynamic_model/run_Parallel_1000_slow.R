...
# 
# Execution code to run all vaccine and incidence simulations under slow-waning assumptions
#
...
for(i in 1:3){
  ...
  if(i == 1){   
  load(paste0("~/Transmission_dynamic_model/","search_grid_slow.Rdata"))                                          # load 1000 samples for stochastic parameters
    scenes = c('A','B','C','D','E','F','G','H','I','J','K','L','M','N','O','P','Q')           # run all vaccine scenarios

    source(here("parallelization_slow.R"))                                                         # load in "run_simulations_parallel" function
    ...
    all_results <- foreach(scene = scenes,.export = c("tspan","scenes", "here"), .packages = c("here", "dplyr")) %dopar% {  
      ...
      for (p in 1:num_runs) {
        ...
        log_message <- paste0("Scenario ", arch, scene, " Slow | ", p, " of ", num_runs,  " | Duration: ", round(duration, 2), " seconds", " | Time Stamp: ", Sys.time())
    ...
    for(idx in seq_along(all_results)) {
      ...
      scene <- scenes[idx]
      results_var_name <- paste0("Scenario_Medium", scene, "_Slow_Results")
      params_var_name <- paste0("Scenario_Medium", scene, "_Slow_Params")
      
      # Construct the file name dynamically for the results and parameters
      file_name_results <- paste0("~/Transmission_dynamic_model/Output/Scenario_Medium", scene, "_Slow_Results.Rdata")
      file_name_params <- paste0("~/Transmission_dynamic_model/Output/Scenario_Medium", scene, "_Slow_Params.Rdata")
